...
def roulette(weights):
    ...
    # Selecionar de forma aleatoria
    # Obs: essa implementacao tem varias limitacoes
...
def random_walk(dim, UB, LB, X_antlion, t, t_max):
    ...
    # Atribuir o raio de reducao progressiva do espaco de busca
    if t > t_max*(0.95):
        I = 1 + (10**6)*(t/t_max)
    elif t > t_max*(0.90):
        I = 1 + (10**5)*(t/t_max)
        ...
        I = 1
    # Mover o espaco de busca e atribuir aleatoriam um quadrante em torno da formiga-leao (armadilha)
    # Obs: a forma como essa parte foi implementada permite violar o espaco de busca inicial (!)
...
#%% ALO
def alo(fobj, UB, LB, n, t_max, seed=3320):
    ...
    sorted_antlion_fitness = F_antlions.copy() # ERRO!!! sorted_antlion_fitness nao eh reordenado!
    X_elite_antlion = X_antlions[0,:].copy()
    f_elite_antlion = F_antlions[0].item()
    
    pops.append(X_antlions)
    ...
    while(t < t_max):
        ...
        for i in range(n):            
            ...
            RE = random_walk(dim, UB, LB, X_elite_antlion, t, t_max) # ERRO!!! sorted_antlion_fitness nao eh reordenado!
        ...
        F_combined = np.concatenate((sorted_antlion_fitness, F_ants)) # ERRO!!! sorted_antlion_fitness nao eh reordenado!
        X_antlions = X_combined[F_combined.argsort()[:n],:]
        F_antlions = np.sort(F_combined)[:n]
        
        # Atualizar a elite
        if F_antlions[0].item() < f_elite_antlion:
            X_elite_antlion = X_antlions[0]
            f_elite_antlion = F_antlions[0].item()
            
        # Mantem a elite na populacao (tb nao entendi)
